...
from math import exp, pi, gamma, log
...
def run(random_seed,
    ...
    delta = 0.1
    alpha = 0.1
    sigma = 6*np.sqrt(2*log(2.5/delta))/epsilon
    warm_up = int(3e3)
    I = np.eye(d)
    B = (exp(epsilon) + 1)/(exp(epsilon) - 1) * np.sqrt(pi) / 2 * d * gamma((d - 1)/2 + 1)/ gamma(d/2 + 1)
    threshold = exp(epsilon)/(exp(epsilon)+1)
    zeta = d*k/(np.sqrt(T)*1000)
    ...
    # estimator
    init_theta = np.random.normal(0, 1, (d))
    init_theta /= np.sqrt(init_theta.T.dot(init_theta))
    init_theta = np.tile(init_theta, (d,k))

    theta_sgd = copy.deepcopy(init_theta)
    theta_ols = copy.deepcopy(init_theta)

    theta_ucb = np.random.normal(0, 1, (d*k, 1))
    theta_ucb /= np.sqrt(theta_ucb.T.dot(theta_ucb))

    theta_gloc = copy.deepcopy(theta_ucb)

    theta_gloc_h = copy.deepcopy(theta_ucb)

    # theta setting
    # theta = np.zeros((d, k))
    # for i in range(k):
    #     theta[i*s:(i+1)*s, i] = 1
    #     theta[:, i] = np.random.normal(0, np.sqrt(v)/3, (d))
    ...
    # context /= np.sqrt(context.T.dot(context))
    ...
    for t in iterator:
        ...
        upsilon_t1 = sigma*np.sqrt(t)*(4*np.sqrt(d) + 2*log(2*T/alpha)) # t-1
        upsilon_t = sigma*np.sqrt(t+1)*(4*np.sqrt(d) + 2*log(2*T/alpha)) # t-1
        c_t1 = 2*upsilon_t1
        c_t = 2*upsilon_t
        beta = 2*sigma*np.sqrt(d*log(T)) + (np.sqrt(3*upsilon_t) + sigma*np.sqrt(d*(t+1)/upsilon_t))*d*log(T)
...
#         optimal_value = np.max([context.T.dot(theta[:, i]) for i in range(k)])
        ...
        if t<warm_up:
            ...
            while True:
                ...
                if (((prob>threshold) and (z.T.dot(x)>0)) or ((prob<=threshold) and (z.T.dot(x)<=0))):
                    break
            eta = np.sqrt(k)*d/(200*(t+1))
            for i in range(k):
                theta_sgd[:, i] = theta_sgd[:, i] - eta*(z[(i*d): ((i+1)*d)].ravel() + 0.00*theta_sgd[:, i])
    ...
    df.to_csv(prefix + 'multi_param_nowarmup' + '|'+ 'eps=' + str(epsilon) +  '|'+ 'n_action=' + str(k) + '|' + 'random_seed=' + str(random_seed) + '|.csv')
    return None
...
if __name__ == "__main__":
    ...
    parser.add_argument('--echo_freq', type = int, default=100,
                        help='echo frequency')
    parser.add_argument('--dest', type = str, default = '/results/multi-param-eps=0.5/',
    ...
    plotters.multiparam_exp_plot(prefix[0], 'Multiparam', 'r')
    plotters.multiparam_exp_plot(prefix[0], 'Multiparam', 'est')

...
class NPUCBserver(Server):
    ...
    def update_params(self):
        self.beta = self.l**(1/2)+np.sqrt(2*log(1/self.alpha)+self.d*log(1+self.t/(self.l*self.d)))
...
class LDPUCBServer(Server):
    ...
    def decide(self, contexts):
        ...
        for i in range(contexts.shape[1]):
            x = contexts[:,i]
            UCB_value = self.theta.T.dot(x)[0]+self.beta*np.sqrt(x.T.dot(temp_matrix).dot(x))
            action_values.append(UCB_value)
        select_index = np.argmax(action_values)
        select_context = contexts[:, select_index].reshape((-1,1))
        return select_index, select_context
    ...
    def update_params(self):
        ...
        self.beta = 2*self.sigma*np.sqrt(self.d*log(self.T)) + (np.sqrt(3*self.upsilon) + self.sigma*np.sqrt(self.d*self.t/self.upsilon))*self.d*log(self.T)
...
#         self.theta = self.theta/np.sqrt(self.theta.T.dot(self.theta))
    ...
    def update(self, t, outputs):
        ...
        eta = (np.sqrt(self.K)*self.d)/(np.sqrt(t+1)*self.eta_constant)
...
#         self.theta_t = self.theta_t/np.sqrt(self.theta_t.T.dot(self.theta_t))
        ...
        self.zeta = (self.d*self.K)/np.sqrt(self.T)
        self.reward = params['env']['yx']
        print(self.reward)
        if self.reward == 'normal':
            self.transform = lambda x: x
...
class LDPCovariateServer(Server):
    ...
    def initialize(self, params):
        ...
        for i in range(self.k):
            self.theta[:, i] /= np.sqrt(self.theta[:, i].T.dot(self.theta[:, i]))

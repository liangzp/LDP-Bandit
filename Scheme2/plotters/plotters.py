...
def matching_files(files, exp=None, **filters):
    return [
        path for path in files
        if (exp is None or exp_name(path) == exp_name(exp)) and settings_match(path, **filters)
    ]
...
def single_exp_plot(repo, label = 'cum_regrets'):
    ...
    for j, eps in enumerate(settings_dict['eps']):
        ...
        for i, exp in enumerate(settings_dict['exp']):
            datas = [pd.read_csv(data) for data in matching_files(files, exp=exp, eps=eps)]
            plot_curve(
                ax,
                datas,
                color = COLORS[i],
    ...
    for label in ['cum_regrets','estimation_error']:
        ...
        for k, reward in enumerate(rewards):
            for j, eps in enumerate(epss):
                ...
                for i, exp in enumerate(settings_dict['exp']):
                    datas = [pd.read_csv(data) for data in matching_files(files, exp=exp, eps=eps, reward=reward)]
                    plot_curve(
                        ax,
                        datas,
                        color = COLORS[i],
...
def ees_exp_plot(repo, title):
    ...
    n_actions = settings_dict['n_action']
    epss = settings_dict['eps']
    files = csv_files(repo, required_keys={'n_action'})
    for k, n_action in enumerate(n_actions):
        for j, eps in enumerate(epss):
            ax = fig.add_subplot(len(n_actions), len(epss), k*len(epss) + (j + 1))
            plotted = False
            for i, exp in enumerate(settings_dict['exp']):
                datas = [pd.read_csv(data) for data in matching_files(files, exp=exp, eps=eps, n_action=n_action)]
                if not datas:
                    continue
                plotted = True
                plot_curve(
